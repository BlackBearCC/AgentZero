...
属性类别：
基础属性：角色最基本的设定信息，包括姓名、年龄、性别、工作职业等固有特征。
...
语言风格：角色语言风格特征。
        ...
        1. 内容结构：每条内容必须包含关键词数组、描述文本和重要程度。
           - 关键词：用于检索和匹配，包含10-50个关联词，覆盖内容的重要概念。
             * 关键词应该是联想触发词而不是和内容直接相关的，关键词避免重复
             * 联想触发词：通过联想和泛化可能触发该内容的词语，可以在语义上疏远但聊到某些内容时人类会联想到，用于提高检索召回率
           - 内容：角色特征的具体描述，长度为10-30字，可包含少量细节补充说明，使用()拼接。
    ...
    def _load_new_attribute_prompt(self):
        """加载新属性生成提示词模板"""
        return Template(f"""
        你是一个专业的角色配置生成专家。请基于以下信息生成新的一个角色属性：
